...
CHUNKS_DIR = os.path.join(BASE_DIR, "data", "experiment_chunks")  # 新的实验目录
os.makedirs(CHUNKS_DIR, exist_ok=True)

CLEANED_PATH = os.path.join(CLEANED_DIR, "merged", "merged_cleaned_documents.json")


# ========= 实验 chunking 超参数 =========
EXPERIMENT_TARGET_LENGTHS = [300, 400, 500, 600, 700, 800, 900, 1000]
HARD_MAX_CHARS = 1200       # 保持原有的硬上限
MIN_CHUNK_CHARS = 200       # 保持原有的最短阈值
OVERLAP_SENTENCES = 1       # 保持原有的重叠句数
SINGLE_SENT_MAX = 400       # 保持原有的单行最长
...
def chunk_one_document(doc: Dict[str, Any], target_length: int) -> List[Dict[str, Any]]:
    """
    对单篇文档进行 chunking：
    ...
    for sent in sentences:
        ...
        if current_len + s_len + 1 <= target_length:
...
def basic_chunk_stats(chunks: List[Dict[str, Any]], target_length: int):
    """打印 chunk 长度分布，检查是否合理。"""
    lengths = [len(c.get("text", "") or "") for c in chunks]
    lengths = [L for L in lengths if L > 0]
    if not lengths:
        print(" 没有可统计的 chunk 长度。")
        return
    ...
    print(f"\n====== Chunk 长度统计（目标长度: {target_length} 字符）======")
    print(f"chunk 总数：{n}")
...
if __name__ == "__main__":
    if not os.path.exists(CLEANED_PATH):
        raise FileNotFoundError(
            f"找不到 {CLEANED_PATH}，请先确保 clean_documents.py 已执行"
        )
    ...
    print(f"共 {len(cleaned_docs)} 篇清洗后的文档")

    # 为每个目标长度创建独立的 chunk 库
    for target_length in EXPERIMENT_TARGET_LENGTHS:
        print(f"\n{'='*50}")
        print(f"正在创建 corpus_{target_length}...")
        print(f"{'='*50}")

        # 创建特定长度的目录
        corpus_dir = os.path.join(CHUNKS_DIR, f"corpus_{target_length}")
        ...
        for idx, doc in enumerate(cleaned_docs, start=1):
            doc_chunks = chunk_one_document(doc, target_length)
        ...
        print(f" corpus_{target_length} 创建完成，共 {len(all_chunks)} 个 chunk")
        print(f"   保存路径: {corpus_path}")

        # 统计长度分布
        basic_chunk_stats(all_chunks, target_length)

    print(f"\n{'='*50}")
    print(f" 所有 6 个 chunk 库创建完成！")
